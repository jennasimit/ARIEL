...
# nc=1 for non-consensus qs analysis, othrwise consensus qs analysis

# input: 
# pheno: N-vector of case-control status (1=case, 0=controls)
...
if(max(y)==2) { # cases=1,controls=2 instead of 1,0
y[y==2] <- 0
}
...
MAFcases <-apply(j.mat,1,MAF.fn,g=g[,cases]) 

ind.rvs <- which(MAFcases < mafrv)
...
print(dim(g)) 
print(dim(qgeno))
grv <- g[ind.rvs,] # genotypes of rare variants in cases
qrv <- qgeno[ind.rvs,]
...
len.fn <- function(g) return(sum(g!="NA",na.rm=TRUE)) # number of variants successfully typed (no filtering on SNP quality)
...
print("reg")	
oreg <- glm(y~prop,family=binomial)
o2pval <- summary(oreg)$coefficients[2,4]
o2beta <- summary(oreg)$coefficients[2,1]
se <- summary(oreg)$coefficients[2,2]
o2OR <- exp(o2beta)
o2ORadj <- exp(o2beta/mrv)
Lo2adj <- exp(o2beta/mrv-1.96*se/mrv)	
Uo2adj <- exp(o2beta/mrv+1.96*se/mrv)
								
# quality score weighted proportion of number of variants with at least 1 minor allele, for indiv i
wp.fn <- function(g,qgeno,i) return(sum((g[,i]>0)*qgeno[,i],na.rm=TRUE)/sum(qgeno[,i],na.rm=TRUE)) 
i.mat <- matrix(1:n,nrow=n)
wp <- apply(i.mat,1,wp.fn,g=grv,qgeno=qrv) # n vector
							
								
wreg <- glm(y~wp,family=binomial)
w2pval <- summary(wreg)$coefficients[2,4]
w2beta <- summary(wreg)$coefficients[2,1]
se <- summary(wreg)$coefficients[2,2]
w2OR <- exp(w2beta)
w2ORadj <- exp(w2beta/mrv)
Lw2adj <- exp(w2beta/mrv-1.96*se/mrv)	
Uw2adj <- exp(w2beta/mrv+1.96*se/mrv)
...
out <- c(mrv,o2pval,w2pval,o2ORadj,w2ORadj,Lo2adj,Lw2adj,Uo2adj,Uw2adj,o2OR,w2OR)
